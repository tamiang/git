...
static int is_object_in_pack_data(struct object_id *oid, void *data UNUSED)
{
	return !! packlist_find(&to_pack, oid);
}
...
static void read_object_list_from_stdin(void)
{
	...
	revs.filter.filter_fn = is_object_in_pack_data;
	...
	for (;;) {
		...
		if (oid_object_info_extended(the_repository, &oid, &oi, 0) < 0) {
			die(_("could not get type of object %s"),
			    oid_to_hex(&oid));
		} else if (type == OBJ_COMMIT) {
			/*
			 * commits in included packs are used as starting points for the
	...
	traverse_commit_list(&revs,
			     show_commit_objects_hint,
			     show_object_objects_hint,
			     NULL);

	trace2_data_intmax("pack-objects", the_repository, "stdin_objects_found",
			   objects_found_nr);
	trace2_data_intmax("pack-objects", the_repository, "stdin_objects_hints",
			   stdin_packs_hints_nr);
}
