...
//            in the rev-list output are ignored.
//
//            <get-missing-options>:
//
...
//
//            TODO describe command verbs, formats, and responses...
//            
...
static const char * const main_usage[] = {
	...
	N_("git gvfs-helper [<main_options>] get-missing [<options>]"),
 ...
 * do something like "rev-list | grep | sed | gvfs-helper".
 */
static unsigned long read_stdin_from_rev_list(struct oidset *oids, int b_missing_only)
{
...
static void lookup_main_creds(void)
{
	...
	trace2_printf("remote_cred [host '%s'][user '%s']",
		      gh__global.main_creds.host,
		      gh__global.main_creds.username ?
		      gh__global.main_creds.username : "");
}
...
static struct tempfile *create_tempfile_for_loose(const struct object_id *oid)
{
	...
	if (!file_exists(buf_path.buf) && mkdir(buf_path.buf, 0777) == -1) {
		error("cannot create directory for loose object '%s'",
		      buf_path.buf);
		goto cleanup;
	}
...
static void install_loose(struct gh__response_status *status,
			  struct tempfile **pp_tempfile,
			  struct strbuf *output_loose_path)
{
	...
	strbuf_setlen(output_loose_path, 0);
	...
	strbuf_add(output_loose_path, tmp_path.buf, len_base);
	...
	// TODO loose object as possibly being in use by another process or as a
	// TODO file that should NEVER need to be updated.)
	// TODO
	// TODO But this prevents us from silently fixing a broken file.
	// TODO
	// TODO Consider lstat'ing the target loose object before we fetch it
	// TODO to avoid the wasted download costs.
	// TODO
	// TODO Alternatively, consider moving the existing loose file to a .bak
	// TODO and then installing it.

	if (finalize_object_file(tmp_path.buf, output_loose_path->buf)) {
		unlink(tmp_path.buf);
		strbuf_addf(&status->error_message,
			    "could not install loose object '%s'",
			    output_loose_path->buf);
		status->ec = GH__ERROR_CODE__COULD_NOT_INSTALL_LOOSE;
		strbuf_setlen(output_loose_path, 0);
		goto cleanup;
	}
 ...
 * and write it to the ODB.  Returns the pathname to the new
 * loose object.
 */
static void do__loose__gvfs_object(
	struct gh__response_status *status,
	const struct object_id *oid,
	struct strbuf *output_loose_path)
{
	...
	params.tempfile = create_tempfile_for_loose(oid);
	if (!params.tempfile) {
		strbuf_addstr(&status->error_message,
	...
	if (status->ec == GH__ERROR_CODE__OK)
		install_loose(status, &params.tempfile, output_loose_path);
			    ...
			    struct string_list *result_list)
{
	...
	for (k = 0; k < nr_total; k += nr_taken) {
		strbuf_setlen(&output_pathname, 0);

		if (nr_total - k == 1 || gh__cmd_opts.block_size == 1) {
			oid = oidset_iter_next(&iter);
			nr_taken = 1;

			do__loose__gvfs_object(status, oid, &output_pathname);
			...
			strbuf_setlen(&msg, 0);
			strbuf_addf(&msg, "loose %s %s", oid_to_hex(oid),
				    output_pathname.buf);
			...
			strbuf_setlen(&msg, 0);
			strbuf_addf(&msg, "packfile %ld %s", nr_taken,
				    output_pathname.buf);
			string_list_append(result_list, msg.buf);
		}
	}
...
/*
 * Read a list of missing objects from stdin (assuming it comes from
 * `rev-list` and has '?' markers for the missing objects) and then
 * fetch them in a single request (or multiple block-size requests).
 */
static enum gh__error_code do_sub_cmd__get_missing(int argc, const char **argv)
{
...
/*
 * Listen to stdin for a series of commands and execute them.  This is an
 * interactive version of get-missing where we let the client (git.exe)
 * send a series of requests (each containing one or more OIDs) and fetch
 * them.  We print one or more results on stdout (this is more for debugging
 * than anything else).
 */
